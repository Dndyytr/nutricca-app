  ...
  return (
    <aside className="hidden md:flex fixed top-0 left-0 h-screen w-[220px] bg-slate-50 border-r border-slate-200 flex-col z-[100] overflow-y-auto">
      ...
      <div className="px-[18px] py-5 border-b border-slate-200 flex items-center gap-2.5 flex-shrink-0">
        ...
        <div>
          <div className="text-[14px] font-bold text-slate-900 leading-tight tracking-tight">Nutricca</div>
          <div className="text-[10px] text-slate-400 mt-0.5 font-medium">Health Tracker App.</div>
        </div>
      </div>
        ...
        {NAV_ITEMS.map(({ path, label, icon }) => {
          ...
          return (
            <Link
              key={path}
              to={path}
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-[9px] text-[13px] mb-0.5 transition-all duration-150 ${
                active ? 'bg-green-50 text-green-700 font-semibold' : 'text-slate-500 font-medium hover:bg-slate-100 hover:text-slate-800'
              }`}
            >
              <span className={active ? 'text-green-600' : 'text-current'}>{icon}</span>
              {label}
            </Link>
          );
        })}
      ...
      <div className="px-3.5 py-4 border-t border-slate-200">
        ...
        >
          <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0">
            {initials}
          </div>
          <div className="min-w-0">
            <div className="text-[12px] font-semibold text-slate-900 truncate">{userProfile?.fullName || 'User'}</div>
            <div className="text-[10px] text-slate-400">{userProfile?.bmiCategory || 'BMI Normal'}</div>
          </div>
        </div>
        <button
          onClick={logout}
          className="w-full py-1.5 px-3 bg-red-50 text-red-600 border border-red-200 rounded-lg text-[12px] font-semibold flex items-center justify-center gap-1.5 hover:bg-red-100 transition-colors"
        >
