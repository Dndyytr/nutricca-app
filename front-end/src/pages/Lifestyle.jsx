...
import { updateOnboardingStep } from '../services/api';
...
export const Lifestyle = () => {
  ...
  const savedDraft =
    JSON.parse(sessionStorage.getItem('draft_lifestyle')) || {};

  const [form, setForm] = useState({
    dietaryPattern: savedDraft.dietaryPattern || 'High Protein',
    mealsPerDay: savedDraft.mealsPerDay || 3,
    dailyWaterIntakeGoal: savedDraft.dailyWaterIntakeGoal || 2000,
    avgSleepHours: savedDraft.avgSleepHours || 7,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      sessionStorage.setItem('draft_lifestyle', JSON.stringify(payload));
      await updateOnboardingStep(3);

      skipToStep('health-security');
      closeFeedback();
      await showSuccess(
