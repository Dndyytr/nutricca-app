...
export const Register = () => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    setLoading(true);
    showLoading('Mengirim OTP...', t('auth.register.setup'));
  ...
  const handleResendOtp = async () => {
    ...
    setError('');
    setLoading(true);
    showLoading('Mengirim ulang OTP...', t('auth.register.setup'));
    try {
      await requestOtp(formData.email);
      closeFeedback();
      startResendCooldown();
  ...
  const handleLoginWithGoogle = async () => {
    ...
    try {
      ...
      window.google.accounts.id.initialize({
        client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID, // Pastikan ini ada di .env React
            ...
            <>
              ...
              <div className="mt-4">
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl font-bold bg-white border-2 border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all"
                >
                  <img
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    maxLength={6}
                    autoFocus
                    className="w-full px-4 py-3.5 rounded-xl bg-slate-100 border-2 border-transparent text-slate-900 text-center tracking-[0.5em] font-bold placeholder:text-slate-400 placeholder:tracking-normal placeholder:font-medium focus:bg-white focus:border-green-500 focus:ring-4 focus:ring-green-500/10 transition-all duration-200 outline-none"
                  />
                </div>
                ...
                >
                  {loading ? t('auth.register.loading') : 'Verifikasi & Daftar'}
                </button>
              </form>
