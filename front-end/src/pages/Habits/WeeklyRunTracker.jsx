...
export const WeeklyRunTracker = () => {
  ...
  const totalKm = DAYS.reduce(
    (sum, d) => sum + (parseFloat(runs[d]?.km) || 0),
    0,
  );
    ...
    <div className="flex flex-col gap-5 font-sans">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
        <div className="flex justify-between items-center mb-3">
          <div className="t-size3 font-bold text-slate-900 tracking-tight">
            {t("habits.weeklyRun.title")}
          </div>
      ...
      <SaveBar
        ...
        status={status}
        errorMsg={errorMsg}
        label={t(runId ? "habits.weeklyRun.update" : "habits.weeklyRun.save")}
      />
    </div>
  );
};
